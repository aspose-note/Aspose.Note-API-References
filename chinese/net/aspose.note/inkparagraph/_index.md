---
title: "类 InkParagraph"
second_title: "Aspose.Note for .NET API 参考"
description: "Aspose.Note.InkParagraph 类。表示包含手写文本的墨水节点，并具有诸如倾斜书写等附加属性"
type: docs
weight: 350
url: /zh/net/aspose.note/inkparagraph/
---
## InkParagraph class

表示包含手写文本且具有倾斜书写等附加属性的墨迹节点。

```csharp
public sealed class InkParagraph : InkNode, IOutlineElementChildNode
```

## 属性

| 名称 | 描述 |
| --- | --- |
| [Document](../../aspose.note/node/document/) { get; } | 获取节点的文档。 |
| virtual [IsComposite](../../aspose.note/node/iscomposite/) { get; } | 获取一个值，指示此节点是否为复合节点。如果为 true，则该节点可以有子节点。 |
| [NextSibling](../../aspose.note/node/nextsibling/) { get; } | 获取同一节点树层级的下一个节点。 |
| [NodeType](../../aspose.note/node/nodetype/) { get; } | 获取节点类型。 |
| [ParentNode](../../aspose.note/node/parentnode/) { get; } | 获取父节点。 |
| [PreviousSibling](../../aspose.note/node/previoussibling/) { get; } | 获取同一节点树层级的上一个节点。 |

## 方法

| 名称 | 描述 |
| --- | --- |
| override [Accept](../../aspose.note/inkparagraph/accept/)(DocumentVisitor) | 接受节点的访问者。 |

### 另见

* class [InkNode](../inknode/)
* interface [IOutlineElementChildNode](../ioutlineelementchildnode/)
* namespace [Aspose.Note](../../aspose.note/)
* assembly [Aspose.Note](../../)


