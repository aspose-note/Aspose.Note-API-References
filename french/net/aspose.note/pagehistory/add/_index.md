---
title: Add
second_title: Référence de l'API Aspose.Note pour .NET
description: Ajoute la version de la page à la fin duPageHistoryaspose.note/pagehistory .
type: docs
weight: 60
url: /fr/net/aspose.note/pagehistory/add/
---
## PageHistory.Add method

Ajoute la version de la page à la fin du[`PageHistory`](../../pagehistory) .

```csharp
public void Add(Page item)
```

| Paramètre | Taper | La description |
| --- | --- | --- |
| item | Page | La version de la page. |

### Voir également

* class [Page](../../page)
* class [PageHistory](../../pagehistory)
* espace de noms [Aspose.Note](../../pagehistory)
* Assemblée [Aspose.Note](../../../)

<!-- DO NOT EDIT: generated by xmldocmd for Aspose.Note.dll -->
